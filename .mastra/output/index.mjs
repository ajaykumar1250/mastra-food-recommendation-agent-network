...
const openrouter$6 = createOpenRouter({
  apiKey: process.env.OPENROUTER_API_KEY
});
const llm = openrouter$6("anthropic/claude-sonnet-4");
const recommendationAgent = new Agent({
  ...
  instructions: `
        ...
        \u{1F37D}\uFE0F PERSONALIZED MEAL RECOMMENDATIONS
        \u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550

        \u{1F4CD} Location: [User's location]
        \u{1F550} Current Time: [Current time for meal context]

        \u{1F3C6} TOP RECOMMENDATIONS
        \u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500

        \u{1F947} [Restaurant Name] - [Cuisine Type]
        \u2B50 Rating: [X.X]/5.0 | \u{1F69A} Delivery: [X-X min] | \u{1F4B0} Price: [Range]
        \u{1F4CD} Address: [Restaurant address]
        
        \u{1F374} RECOMMENDED DISHES:
        ...
        \u{1F948} [Restaurant Name] - [Cuisine Type]
        \u2B50 Rating: [X.X]/5.0 | \u{1F69A} Delivery: [X-X min] | \u{1F4B0} Price: [Range]
        \u{1F4CD} Address: [Restaurant address]
        ...
        \u{1F949} [Restaurant Name] - [Cuisine Type]
        \u2B50 Rating: [X.X]/5.0 | \u{1F69A} Delivery: [X-X min] | \u{1F4B0} Price: [Range]
        \u{1F4CD} Address: [Restaurant address]
...
const searchRestaurantsStep = createStep({
  ...
  execute: async ({ inputData }) => {
    ...
    const { searchRestaurants } = await Promise.resolve().then(function () { return restaurantSearchTool$1; });
...
const generateRecommendationsStep = createStep({
  ...
  execute: async ({ inputData }) => {
    ...
    const { restaurants, userPreferences } = inputData;
...
}).then(searchRestaurantsStep).then(generateRecommendationsStep);
mealSuggestionWorkflow.commit();

const restaurantSearchTool = createTool({
  id: "search-restaurants",
...
const getOpenTripMapRestaurants = async (params) => {
  try {
    ...
    const restaurants = data.features.map((place) => ({
      ...
      isOpen: true
      // Assume open since we don't have this data
    }));
...
const extractCuisineFromTags = (kinds) => {
...
const generatePriceRange = (budget) => {
  switch (budget) {
...
const generatePopularDishes = (categories) => {
  ...
  const dishMap = {
    ...
    french: ["Croissant", "Coq au Vin", "Ratatouille", "Cr\xE8me Br\xFBl\xE9e"]
  };
  return dishMap[cuisine] || ["Chef Special", "House Favorite", "Popular Dish", "Signature Item"];
};
const filterAndSortRestaurants = (restaurants, params) => {
  let filtered = restaurants;
  if (params.cuisine) {
    const cuisineLower = params.cuisine.toLowerCase();
    filtered = filtered.filter((restaurant) => {
      const restaurantCuisine = restaurant.cuisine.toLowerCase();
      return restaurantCuisine.includes(cuisineLower) || cuisineLower.includes(restaurantCuisine) || cuisineLower === "chinese" && restaurantCuisine === "american" || // Include American for Chinese preference
      cuisineLower === "italian" && restaurantCuisine === "american";
    });
  }
  if (params.dietaryRestrictions?.includes("vegetarian")) {
    filtered = filtered.filter(
      (restaurant) => restaurant.cuisine === "Vegetarian" || restaurant.cuisine === "Healthy"
    );
  }
  if (params.budget) {
    filtered = filtered.filter((restaurant) => {
  ...
  if (params.healthPreference === "healthy") {
    filtered = filtered.filter(
      (restaurant) => restaurant.cuisine === "Healthy" || restaurant.cuisine === "Vegetarian"
    );
  }
  return filtered.sort((a, b) => b.rating - a.rating).slice(0, 5);
};

var restaurantSearchTool$1 = /*#__PURE__*/Object.freeze({
  __proto__: null,
  restaurantSearchTool: restaurantSearchTool,
  searchRestaurants: searchRestaurants
});

const openrouter$5 = createOpenRouter({
  apiKey: process.env.OPENROUTER_API_KEY
});
const mealSuggestionAgent = new Agent({
  ...
  `,
  model: openrouter$5("anthropic/claude-sonnet-4"),
  tools: { restaurantSearchTool },
  workflows: { mealSuggestionWorkflow }
});

const openrouter$4 = createOpenRouter({
  apiKey: process.env.OPENROUTER_API_KEY
});
const preferenceCollectionAgent = new Agent({
  ...
  `,
  model: openrouter$4("anthropic/claude-sonnet-4")
});

const openrouter$3 = createOpenRouter({
  apiKey: process.env.OPENROUTER_API_KEY
});
const locationAnalysisAgent = new Agent({
  ...
  `,
  model: openrouter$3("anthropic/claude-sonnet-4")
});

const openrouter$2 = createOpenRouter({
  apiKey: process.env.OPENROUTER_API_KEY
});
const recommendationSynthesisAgent = new Agent({
  ...
  `,
  model: openrouter$2("anthropic/claude-sonnet-4")
});

const openrouter$1 = createOpenRouter({
  apiKey: process.env.OPENROUTER_API_KEY
});
const budgetOptimizationAgent = new Agent({
  ...
  `,
  model: openrouter$1("anthropic/claude-sonnet-4")
});

const openrouter = createOpenRouter({
  apiKey: process.env.OPENROUTER_API_KEY
});
const mealSuggestionNetwork = new NewAgentNetwork({
